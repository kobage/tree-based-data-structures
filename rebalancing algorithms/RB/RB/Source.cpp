...
void balancing(int tree_size, int method)
{
	...
	case 2: cout << "Test started: qBalanceComplete..." << endl << endl; break;
	...
	for (int k = 0; k < REPETITIONS; ++k)
	{
		...
		{
			Node<int>* node = new Node<int>(di(dre));
		...
		switch (method)
		{
		case 1: tree.balanceMod(); break;
		case 2: tree.qBalanceComplete(); break;
		}
		diff = chrono::high_resolution_clock::now() - st;
		time = chrono::duration_cast<chrono::milliseconds>(diff);
...
int main()
{
	...
	cin >> n;
	cout << "To balance with Sedgewick’s algorithm enter 1; to balance with qBalance, enter 2" << endl;
	int choice;
	cin >> choice;
	balancing(n, choice);
}